require([
	...
	var packs = [
		['Core', [
			...
			['Declarative Creation', 'test_grid_declarative', 'declarative.png', 'Declaratively create a grid in HTML by means of dojo parser.'],
		...
		['Plugins', [	
			...
			['Pagination', 'test_grid_paginationBarDD', 'pagination.png', 'Visually paging the rows, let the user control paging details. This is a useful alternative to virtual scrolling.'],
			['Export & Print', 'test_grid_exporter', 'exportprint.png', 'An export framework for grid. Currently only support exporting to CSV format. Also provides print & preview machinery.'],
			...
			['Filter', 'test_grid_filter', 'filter.png', 'Powerful filter bar and filter dialog, supporting advanced client-side or server-side filtering.'],
			['Widgets in Cell', 'test_grid_alwaysEditing', 'cellWidget.png', 'Provides better support for dijits/widgets in grid cells. These widgets are reused to improve performance, regardless whether the virtual scrolling or pagination feature is used.'],
		...
		0]],
		['Performance', [
 		    ['GridX with a huge store(1,000,000 rows)', 'test_grid_huge_data', 'hugestore.png', 'GridX can now work nicely with extremely huge data stores larger than 1 million rows'],
 		0]],		
		['Layout', [
		...
		['Tree Grid (Experimental)', [
 			['Tree Grid', 'test_grid_tree', 'tree.png', '<b>Experimental</b> - Makes grid support Tree structured data. Two types of tree data models are supported: nested or not. In nested mode, expandos are placed in different columns whilist all expandos are in one column in the other mode.'],
 		0]],
