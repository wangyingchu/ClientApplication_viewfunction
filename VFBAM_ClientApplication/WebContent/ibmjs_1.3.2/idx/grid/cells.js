...
define([
    ...
    "dojo/i18n!./nls/cells"
			 ...
			 iSelect,				// (../form/Select)
			 iCheckBoxSelect,		// (../form/CheckBoxSelect)
 ...
 */
iCells.CheckBox = iCells.Checkbox = dDeclare("idx.grid.cells.CheckBox", iCells._Widget,
/**@lends idx.grid.cells.CheckBox#*/
{
	...
	/**
	 * The label to associate with the checkbox.  If none given then the
	 * default is used.
	 */
	labelText: "", 
	 ...
	 */
	formatter: function(value, index){
		var resources = iResources.getResources("idx/grid/cells");
		...
		if(this.allCheckable){
			...
			if(!this._headerWidget){
				...
				var headerWidget = this._headerWidget = new dCheckBox({onClick: checkAll,
						onMouseDown: stop, onMouseUp: stop, title: resources.headerCheckBoxLabel});
				dDomAttr.set(headerWidget.focusNode, "aria-label", resources.headerCheckBoxLabel);				
				var cell = this;
				var grid = this.grid;
				headerWidget.own(dAspect.after(grid, "onKeyDown", function(event){
	...
	_createWidget: function(index){
		...
		var widget = new dCheckBox({onClick: apply, title: resources.rowCheckBoxLabel});
		dDomAttr.set(widget.focusNode, "aria-label", resources.rowCheckBoxLabel);
		widget.own(dAspect.after(grid, "onKeyDown", function(event){
...
/**@lends idx.grid.cells.RowSelector#*/
{
	...
	formatter: function(){
		if(!this._selectionChanged){
			...
			this.grid.own(dAspect.after(this.grid, "onSelectionChanged", this._selectionChanged, true));
		}
...
/**@lends idx.grid.cells.MultiSelect#*/
{
	...
	_createWidget: function(index){
		...
		var widget = new iCheckBoxSelect({store: this.store, placeHolder: this.placeHolder,
...
/**@lends idx.grid.cells.Text#*/
{
	...
	formatNode: function(node, value, index){
		...
		if(node && this.applyOnBlur && this.editable){
			var scope = this;
			this._handle = dAspect.after(node, "onblur", function(){
				setTimeout(function(){
				...
				if (scope._handle) {
					scope._handle.remove();;
					delete scope._handle;
				}
			}, true);
		}
	},
 ...
 * @class Renders an empty cell. This is typically used for a virtual column for a handle to select rows when all other columns are editable by single click.
 * @augments dojox.grid.cells.Cell
 */
