		         ...
		         templateText)	    // (dojo/text!./templates/A11yPrologue.html)
{
	 ...
	 * of the "body" tag. 
	 * @name idx.app.A11yPrologue
	 * @class
	 * @public
	 */
	return dDeclare("idx.app.A11yPrologue", [dWidget,dTemplatedMixin],
	/**@lends idx.app.A11yPrologue#*/	
{
	...
	/**
 	 * The widget template text for the dijit._TemplatedMixin base class.
 	 * @constant
 	 * @type String
 	 * @private
 	 * 
	 ...
	 * @public
	 * @function
	 * @param {Node|String} target The node or anchor string for the shortcut. 
	 * @param {String} description The description of the shortcut.
	 * @param {String} accessKey The access key.
	 * @return {String} The ID of the shortcut for later unregistering it.
	 */
	 ...
	 * @public
	 * @function
	 * @param {String} shortcutID The ID of the shortcut to remove.
	 */
	...
	_setupLink: function(linkType, targetNode, accessKey, description, linkNode, refNode, pos) {
		...
		if (targetNode) {
			...
			var innerHTML = dString.substitute(
					"<a id='${nodeID}' href='#${location}' ${accessKeyAttr}>" +
						this._resources.skipToLocationMessage +	"</a>",
				{
					nodeID: nodeID,
					location: location,
			...
			if (clickableNode) {
				handle = dOn(clickableNode, "click", clickHandler);
				this._linkHandles[linkType] = handle;
			}
	...
	_setupShortcut: function(target, accessKey, description, itemNode, refNode, pos, shortcutID) {
		...
		if (target) {
			...
			if(keyComponent){
				if(shortcutType == "external"){
					messageFormat = messageFormat.replace("${description}",
						"<a href='${location}' ${accessKeyAttr} ${targetWindowAttr}>${description}</a>");
				}else{ // "internal"
					messageFormat = messageFormat.replace("${description}",
						"<a id='${nodeID}' href='#${location}' ${accessKeyAttr}>${description}</a>");
				}
			}else{
				if(shortcutType == "external"){
					messageFormat = "<a href='${location}' ${targetWindowAttr}>" + messageFormat + "</a>"; 
				}else{ // "internal"
					messageFormat = "<a id='${nodeID}' href='#${location}'>" + messageFormat + "</a>";
				}
			}
			...
			if ((clickableNode)&&(clickHandler)) {
				handle = dOn(clickableNode, "click", clickHandler);
				this._shortcutHandles[shortcutID] = handle;
			}
