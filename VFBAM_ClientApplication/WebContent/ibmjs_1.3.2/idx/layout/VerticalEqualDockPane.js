...
], function(dojo_declare, dojo_array, dojo_html, dojo_connect, dojo_base_fx, dojo_fx, dijit_wai, dijit_layout_LayoutWidget, dijit_TitlePane, idx_layout__DockAreaMixin){  
 ...
 */
return dojo_declare("idx.layout.VerticalEqualDockPane", [ dijit_layout_LayoutWidget, idx_layout__DockAreaMixin ],
/**@lends idx.layout.VerticalEqualDockPane#*/		
{
	 ...
	 */
	calcMarginSize: function(pane) {
		if (this.__calculated) return;
		
		// assume all pane has same style, same height title
		// calculate border/margin/padding size first
	 ...
	 */
	setupChild: function(pane) {
		this.calcMarginSize(pane);
		...
		var h1 = pane.connect(pane, "onUndock", function() {
			dojo_html.style(this.wipeNode, "height", "auto");
			dojo_html.style(this.containerNode, "height", "auto");
			this._setOpenAttr = org_setOpenAttr;
			this.disconnect(h1);
		});
		var h2 = pane.connect(pane, "onDock", function() {
			var parent = this.getParent();
	...
	layoutVertical: function() {
		...
		for ( var i = 0; i < info.openPanes.length; i++) {
			...
			if (!dojo_html.hasClass(pane.domNode, "idxDockAreaMockPane")) {
				...
				if (this.scrollable) {
					dojo_html.marginBox(pane.containerNode, {
						h: info.wipeHeight
					});
				}
				availableHeight -= dojo_html.marginBox(pane.domNode).h;
