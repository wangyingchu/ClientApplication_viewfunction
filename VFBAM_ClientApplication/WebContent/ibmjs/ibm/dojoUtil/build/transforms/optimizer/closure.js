	...
	if(has("host-rhino")){
		...
		var ccompile = function(text, dest, optimizeSwitch, copyright){
			...
			var emptyDescription = "",
				compilerFileContent = compiler.toSource();


			if ( compilerFileContent == "" ){
				emptyDescription = "//>>ClosureEmptyFile\n";
				bc.logOptimizerOutput("ClosureEmptyFilePath: " + dest + "\t\n");
			}
			
            var result = copyright + "//>>built" + bc.newline + emptyDescription + compilerFileContent + bc.newline + map_tag;
