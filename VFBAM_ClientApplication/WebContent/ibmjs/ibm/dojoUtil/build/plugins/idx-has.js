	...
	return {
		start:function(
			...
			var
				...
				get = function(skip){
					var operator, term = tokens[i++], staticFeature = false;
					if(term == ":"){
						...
						if(tokens[i++] == "?"){
							staticFeature = (term.length>1 && term.charAt(0)=='#');
							if (staticFeature) term = term.substring(1);
							var hasResult = has(term);
							if ((hasResult === undefined) && staticFeature) hasResult = false;
			...
			var regex = new RegExp("((idx\\/)|([./]+))has\\!" + dojoRegExp.escapeString(id));
			if(!resolvedId){
