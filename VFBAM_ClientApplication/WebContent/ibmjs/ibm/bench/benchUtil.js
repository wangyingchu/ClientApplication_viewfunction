...
"dojox/charting/plot2d/ClusteredBars", "dojox/charting/themes/CubanShirts", "dojox/charting/action2d/Tooltip"], 
...
//            dojo.style(this.domNode, "display", "block");
    ...
    var benchTool = {
			...
			dijit.byId("widgetList")["onChange"] = function(value){
				...
				currentStore.fetchItemByIdentity({
					...
					onItem: function(item){
						...
						$("#singleRunner").button("enable");
						//dijit.byId("compareRunner").set("disabled", false);
						$("#compareRunner").button("enable");
						...
						dojo.forEach(currentTemplates, function(template, idx){
							if(idx == 0){
								...
								theTemplate.jqOptions = currentTemplates[idx].jqOptions;
								theTemplate.jqType = currentTemplates[idx].jqType;
								theTemplate.jqSelector = currentTemplates[idx].jqSelector;
							}
							...
							require(clazz, function(){
								...
								if(template.jqType){
									$("#widgetSamples:last-child "+template.jqSelector)[template.jqType](template.jqOptions);
								}else{
        ...
        createWidget: function(clazz, declaration, count, container, jqType, jqOptions, jqSelector){
            var tmpNode = dojo.create("div", {});
            tmpNode.innerHTML = string.rep(declaration, count);
            this.startTimer = new Date().getTime();
			if(jqType){
				dojo.place(tmpNode, container);
				$("#widgetsContainer>div "+jqSelector)[jqType](jqOptions);
			}else{
	            dojo.parser.parse(tmpNode);
				dojo.place(tmpNode, container);
			}
		...
		clearWidget: function(container){
			...
			dojo.query(".idxDialog, .dijitDialog").forEach(function(node){dijit.byNode(node).destroyRecursive();})
			$("#"+container + " .ui-widget").remove();
			$(".ui-datepicker, .ui-dialog").remove()
		...
		generateCharting: function(){
			...
			dojo.query(".dijitCheckBox","results").forEach(function(node, idx){
				...
				if(ckbox.checked){
					...
					var label = data.description.replace(/Dijit|JQuery|IDX/,'');//this._getWidgetName(data.clazz);
					if(labels.indexOf(label) == -1){
						labels.push(label);
					}
					if(this._isTargetWidget(data.description)){
						targetSeries.push({
							y: data.average,
							tooltip: data.description + ": " + data.average + "ms"
						})
					}else if(this._isDijitWidget(data.description)){
						dijitSeries.push({
							y: data.average,
							tooltip: data.description + ": " + data.average + "ms"
						})
			...
			benchChart.setTheme(dojox.charting.themes.CubanShirts).
			addPlot("default", {
				type: "ClusteredBars",
				gap: 2
			}).addSeries("IDX Widget", idxSeries).addSeries("Dijit Widget", dijitSeries).addSeries("jQuery Widget", targetSeries)
			.addAxis("y", {
				vertical: true, 
...
//				titleOrientation: "away"
		...
		_isTargetWidget: function(desc){
			return desc.indexOf("JQuery") > -1;
		},
		_isDijitWidget:function(desc){
			return desc.indexOf("Dijit") > -1;
		},
		...
		resetRunnerLabel: function(){
			...
			this.currentRunnerButton.button("option", {label: this.runnerLabel, disabled: false});
		},
		/**
		 * Tasks
		 */
		_getSingleTestTask: function(){
			return [{
				...
				jqOptions: theTemplate.jqOptions,
				jqType: theTemplate.jqType,
				jqSelector: theTemplate.jqSelector
			},{type: "resetRunnerLabel"}]
		},
		_getPairTestsTask: function(){
			this.deselectAllReports();
			var tasks = dojo.map(currentTemplates, function(template, idx){
				return {
					clazz: theTemplate.clazz,
					...
					method: "parse",
					container: "widgetsContainer",
					jqOptions: template.jqOptions,
					jqType: template.jqType,
					jqSelector: template.jqSelector
				}
			})
			return tasks.concat([{type: "generatingChart"},{type: "resetRunnerLabel"}]);
		},
				...
				onComplete: function(items){
					dojo.forEach(items, function(item){
						if(item.templates && item.templates[1]){
							if(_this._isTargetWidget(item.templates[1].description)){
		...
		_runner: function(task){
			if(task.type && task.type == "generatingChart"){
				...
				this.createWidget(task.clazz, task.declaration, task.count, task.container, 
					task.jqType, task.jqOptions, task.jqSelector);
				var _this = this;
				setTimeout(function(){
		...
		runSingleTest : function(){
			...
			this.currentRunnerButton = $("#singleRunner"); //dijit.byId("singleRunner");
			this.runnerLabel = this.currentRunnerButton.button("option", "label");
			//this.currentRunnerButton.set("label", "Running...").set("disabled", true);
			this.currentRunnerButton.button("option", {label: "Running...", disabled: true});
			var task = this._getSingleTestTask();
			setTimeout(dojo.hitch(this, function(){this._runRealTest(task); }),1000);
		},
		runPairTests: function(){
			if(this.isRunning){return;}
			this.currentRunnerButton = $("#compareRunner"); //dijit.byId("compareRunner");
			this.runnerLabel = this.currentRunnerButton.button("option", "label");
		...
		_constructTable: function(){
			...
			dojo.forEach(this.masterResults.dataSet,function(data){
				...
				if(!dataSet[widgetName]){
					var o = {};
					//if(_this._isDijitWidget(data.description)){
					//	o.compareWith = data.description;
					//	o.baseCost = data.average;
					//}else 
					if(_this._isTargetWidget(data.description)){
						o.compareWith = data.description;
						o.baseCost = data.average;
					}else{
			...
			for(var i in dataSet){
				var performance = ((dataSet[i].extCost - dataSet[i].baseCost)/dataSet[i].baseCost)*100;
					status = performance > 30 ? "bad" : (performance < -30 ? "good" : "");
				var item = 
					"<tr>"+
						"<td>" + i + "</td>" +
