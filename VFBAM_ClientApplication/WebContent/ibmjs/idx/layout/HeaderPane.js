		         ...
		         templateText) 		// templateText		(dojo/text!./templates/HeaderPane.html)
{
	 ...
	 */
return dDeclare("idx.layout.HeaderPane",[iContentPane,dLayoutWidget,dTemplatedMixin,dCssStateMixin],
		/**@lends idx.layout.HeaderPane#*/	
{
	...
	/**
	 * The html template for the dijit._TemplatedMixin base class.
	 * @constant
	 * @type String
	 * @private
  ...
  destroy: function() {
	...
	dArray.forEach(this.getMajorActionChildren(), function(child) {
		child.destroyRecursive();
	});
	dArray.forEach(this.getMinorActionChildren(), function(child) {
		child.destroyRecursive();
	});
  ...
  startup: function() {
    ...
    if(this._idxStarted){
    	// if the IDX started variable is set then we need only defer to the base 
    	// implementation to potentially restart after deletion this._started
	...
	// Adjust margin box of containerNode for visibility of focus border.
	if(!dHas("ie")){
		dDomStyle.set(this.containerNode, "margin", dHas("safari") ? "3px" : "1px");
	}
  ...
  * @private
  ...
  removeChild: function(child) {
    ...
    if (foundIndex >= 0) {
      var handle = actions[foundIndex].handle;
      if (handle) {
      	handle.remove();
      	delete actions[foundIndex].handle;
      }
