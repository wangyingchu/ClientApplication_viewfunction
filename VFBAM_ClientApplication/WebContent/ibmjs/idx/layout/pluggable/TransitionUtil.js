define(["dojo/_base/lang",
	...
	var translateMethodSlip = "translate3d(";
	if(has("webkit")){
	...
	transition.slip = function(node, config){
		// summary:
		//		Method which is used to create the transition object of a slide effect.
		// node:
		//		The node that the slide transition effect will be applied on.
		...
		if ( translateMethodSlip == "translate3d("){
			ret.startState[transitionPrefix + "ransform"]=translateMethodSlip +"0," + startX+ ",0)";
			ret.endState[transitionPrefix + "ransform"]=translateMethodSlip +"0," + endX + ",0)";
		}
		else{
			ret.startState[transitionPrefix + "ransform"]=translateMethodSlip+startX+translateMethodEnd;
			ret.endState[transitionPrefix + "ransform"]=translateMethodSlip+endX+translateMethodEnd;
		}
	...
	transition.flip = function(node, config){
		...
		if(ret["in"]){
			...
			ret.startState[transitionPrefix + "ransform"]="translateX(100%) rotateY(-180deg)";
			ret.endState[transitionPrefix + "ransform"]="translateX(0) rotateY(0)";
		}else{
			lang.mixin(ret,{
				startState:{
					"opacity": "1"
			...
			ret.startState[transitionPrefix + "ransform"]="translateX(0) rotateY(0)";
			ret.endState[transitionPrefix + "ransform"]="translateX(100%) rotateY(180deg)";
