...
module.exports = function( grunt ) {
	...
	var fs = require( "fs" ),
		...
		config = {
			...
			wrap: {
				startFile: "build/prefix.js",
				endFile: "build/suffix.js"
			},
			paths: {
	...
	function convert( name, path, contents ) {
		...
		// 		.replace( rdefineEnd, "" );
		// }else 
		if(/jui\/util/.test(path)){		// jui/util
			contents = contents
				.replace( /define\([\w\W]*?return/, "var util =" )
				.replace( rdefineEnd, "" );
		...
		}else if( /^text!/.test(name)){		// ignore loading pure text file
			return '';
		}else{
			contents = contents
				...
				// remove "util.execute(code) code block"
				.replace(/\s*if\s*\([^\(\)\{\}]*\)\s*\{\s*util\.execute\(code\);\s*\}/, "");
