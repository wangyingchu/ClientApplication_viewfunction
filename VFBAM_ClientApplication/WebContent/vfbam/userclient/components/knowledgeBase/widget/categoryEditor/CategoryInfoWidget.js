require([
    ...
    declare("vfbam.userclient.components.knowledgeBase.widget.categoryEditor.CategoryInfoWidget", [_Widget, _Templated], {
        ...
        resetCategoryEditorListenerHandler:null,
        ...
        postCreate: function(){
            ...
            this.resetCategoryEditorListenerHandler= Application.MessageUtil.listenToMessageTopic(APP_KNOWLEDGEBASE_RESETCATEGORYEDITOR_EVENT,  dojo.hitch(this,this.resetCategoryEditor));
            ...
            this.removePropertyDropDown=new vfbam.userclient.components.knowledgeBase.widget.categoryEditor.RemoveCategoryPropertyWidget({categoryInfoWidget:this});
        ...
        renderCategoryInfo:function(data){
            dojo.style(this.categoryEditor,"display","");
            /*
            ...
            */
            ...
            this.removePropertyDropDown.renderPropertyList([]);
            ...
            if(categoryMetaInfo.categoryId){
                ...
                this.categoryCodeField.set("value",this.currentCategoryMetaData.categoryCode);
                ...
                if(this.currentCategoryMetaData.categoryPropertys){
                    ...
                    this.removePropertyDropDown.renderPropertyList(this.currentCategoryMetaData.categoryPropertys);
                }else{
        ...
        doUpdateBasicProperties:function(){
            ...
            newPropertiesDataObj["categoryCode"]=this.categoryCodeField.get("value");
            var newPropertiesDataObjContent=dojo.toJson(newPropertiesDataObj);
        ...
        deleteProperties:function(propertiesToDeleteArray){
            if(propertiesToDeleteArray.length==0){
                UI.showToasterMessage({type:"warn",message:"请选择需要删除的业务属性"});
                return;
            }
            var that=this;
            var messageTxt="请确认是否删除选择的业务属性?";
            var confirmButtonAction=function(){
                that._deleteProperties(propertiesToDeleteArray);
            };
            var cancelButtonAction=function(){}
            UI.showConfirmDialog({
                message:messageTxt,
        ...
        _deleteProperties:function(propertiesToDeleteArray){
            var currentCategoryProperties=this.currentCategoryMetaData.categoryPropertys;
            var remainCategoryPropertiesArray=[];
            dojo.forEach(currentCategoryProperties,function(currentProperty){
                if(this.isPropertyNeedDelete(currentProperty.propertyName,propertiesToDeleteArray)){
                    remainCategoryPropertiesArray.push({
                        propertyName:currentProperty.propertyName,
                        propertyType:currentProperty.propertyType,
                        propertyValue:null
                    });
                }else{
                    remainCategoryPropertiesArray.push(currentProperty);
                }
            },this);
            var that=this;
            var newPropertiesDataObj={};
            newPropertiesDataObj["categoryNodeLocation"]= this.currentCategoryMetaData.categoryNodeLocation;
            newPropertiesDataObj["categoryPropertys"]=remainCategoryPropertiesArray;
            var newPropertiesDataObjContent=dojo.toJson(newPropertiesDataObj);
            var resturl=KNOWLEDGE_CATEGORY_SERVICE_ROOT+"updateCategoryProperties/";
            var errorCallback= function(data){
                UI.showSystemErrorMessage(data);
            ...
            var loadCallback=function(data){
                UI.showToasterMessage({type:"success",message:"删除分类业务属性成功"});
                that.renderCategoryInfo({category:data});
                that.removePropertyMenuDialog.close();
                Application.MessageUtil.publishMessage(APP_KNOWLEDGEBASE_UPDATECATEGORYDATA_EVENT,{category:data});
            };
            Application.WebServiceUtil.postJSONData(resturl,newPropertiesDataObjContent,loadCallback,errorCallback);
        },
        isPropertyNeedDelete:function(propertyName,propertyNeedDeleteArray){
            var result=false;
            dojo.forEach(propertyNeedDeleteArray,function(propertyNeedDelete){
                if(propertyNeedDelete==propertyName){
                    result=true;
                }else{
                }
            });
        ...
        resetCategoryEditor:function(data){
            if(data.categoryData.id==this.currentCategoryData.id){
                dojo.style(this.categoryEditor,"display","none");
            }
        },
        destroy:function(){
            this.categorySelectedListenerHandler.calcelMessageListening();
            this.resetCategoryEditorListenerHandler.calcelMessageListening();
