require([
    ...
    "dojo/text!vfbam/userclient/components/enterpriseInfoManagement/widget/userList/template/UserListWidget.html",
    "dojo/dom-class"
],function(lang,declare, _Widget, _Templated, template,domClass){
    declare("vfbam.userclient.components.enterpriseInfoManagement.widget.userList.UserListWidget", [_Widget, _Templated], {
        ...
        loadUserList:function(selectedUserId){
            var enterpriseLDAPNode="enterpiseInfo";
            var that=this;
            var resturl=PARTICIPANT_SERVICE_ROOT+"participantOperationService/usersInfo/detailInfo/"+enterpriseLDAPNode;
            var syncFlag=true;
            ...
            var loadCallback=function(restData){
                if(restData){
                    ...
                    Application.MessageUtil.publishMessage(APP_ENTERPRISE_UPDATEUSERTOTALNUMBER_EVENT,{
        ...
        renderUserList:function(userRole,selectedUserId){
            ...
            if(userRole){
                dojo.forEach(this.applicationSpaceUserMetaDataArray,function(userData){
                    if(userData.roleType==userRole){
                        var currentUserBasicInfoItem=new vfbam.userclient.components.enterpriseInfoManagement.widget.userList.UserBasicInfoMagazineViewItemWidget({
            ...
            if(selectedUserId){
                ...
                if(that.userInfoItemsArray.length>0){
                    ...
                    Application.MessageUtil.publishMessage(APP_ENTERPRISE_USERINFOSELECTED_EVENT,{userDetailInfo:null,selectedUserInfoWidget:null});
                }
            }
        },
        filterEnterpriseByIndustry:function(industry,selectedUserId){
            this._cleanDirtyItemData();
            var isOdd=true;
            var that=this;
            dojo.forEach(this.applicationSpaceUserMetaDataArray,function(userData){
                if(userData.title==industry){
                    var currentUserBasicInfoItem=new vfbam.userclient.components.enterpriseInfoManagement.widget.userList.UserBasicInfoMagazineViewItemWidget({
                        userDetailInfo:userData,currentSelectedUserInfoItemArray:that.currentSelectedUserInfoItemArray});
                    if(isOdd){
                        domClass.add(currentUserBasicInfoItem.domNode, "app_magazineView_item_odd");
            ...
            return this.userInfoItemsArray.length;
        },
        filterUsersByProperty:function(userSearchScope,searchOption,searchValue){
            ...
            dojo.forEach(this.applicationSpaceUserMetaDataArray,function(userData){
                ...
                if(matchFilter&&userSearchScope!=APP_ENTERPRISE_ALLUSERSEARCH_CONST){
