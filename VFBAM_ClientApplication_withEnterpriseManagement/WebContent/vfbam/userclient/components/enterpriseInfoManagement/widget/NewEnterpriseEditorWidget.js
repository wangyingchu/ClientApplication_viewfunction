require([
    ...
    "dojo/text!vfbam/userclient/components/enterpriseInfoManagement/widget/template/NewParticipantEditorWidget.html","idx/oneui/Dialog"
],function(lang,declare, _Widget, _Templated, template,Dialog){
    declare("vfbam.userclient.components.enterpriseInfoManagement.widget.NewEnterpriseEditorWidget", [_Widget, _Templated], {
        ...
        addNewUser:function(){
            ...
            var messageTxt="请确认是否添加企业信息 <b>"+this.displayNameField.get("value")+" - "+this.userIdField.get("value")+"</b> .";
            var that=this;
        ...
        _doAddUserProfile:function(){
            ...
            var loadCallback=function(data){
                UI.hideProgressDialog();
                if(data){
                    UI.showToasterMessage({type:"success",message:"添加企业信息成功"});
                    if(that.callback){
                        that.callback(data);
                    }else{
