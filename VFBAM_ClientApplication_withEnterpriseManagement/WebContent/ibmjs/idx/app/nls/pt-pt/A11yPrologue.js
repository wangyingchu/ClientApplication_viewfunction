define({     
//begin v1.x content
	keySequence_InternetExplorer: "ALT + ${accessKey}",
	keySequence_Firefox: "ALT + SHIFT + ${accessKey} em Windows e Linux ou CONTROL + ${accessKey} em Mac",
	keySequence_Safari: "CONTROL + OPT + ${accessKey} em Mac ou ALT + ${accessKey} em Windows",
	keySequence_Chrome: "ALT + ${accessKey} em Windows e Linux ou CONTROL + OPT em Mac",
	shortcutListMessage: "Os atalhos para esta página são:",
	a11yPrologueLabel: "Prólogo de acessibilidade",
    a11yStatementLabel: "Declaração de acessibilidade",
    skipToLocationMessage: "Avançar para ${description}",
	shortcutKeyMessage_internal: "Para avançar para ${description} utilize ${keySequence}.",
	shortcutKeyMessage_external: "Para ligar a ${description} utilize ${keySequence}.",
	shortcutMessage_internal: "Avançar para ${description}.",
	shortcutMessage_external: "Ligar a ${description}.",

	a11yMainContentAreaName: "conteúdo principal",

	a11yNavigationAreaName: "navegação",

	a11yBannerAreaName: "faixa"
//end v1.x content
});

