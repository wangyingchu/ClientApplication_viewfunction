define(["dojo/_base/lang","dojo/_base/html","dojo/_base/connect","dojo/_base/array","dojo/_base/window","dojo/_base/xhr", // dojo Base
		...
		"./src/Viewport",
		"./src/structure"],
  function(lang, html, connect, array, win, xhr, dom, domClass,domProp, domConstruct, hash,
			Deferred, DeferredList, ItemFileReadStore, Memory, registry, highlight, jshighlight, compat, parser, dm,
	...
	function navBtnClickHandler() {
		...
		if (navRecord.fromTitle)
			registry.byId("header").set("label", navRecord.fromTitle);
	}
		...
		connect.connect(view, "onAfterTransitionIn", view, function(){
			...
			if (viewType === 'demo') {
				...
				if (!transitFrom || (transitFrom != "source")) {
					registry.byNode(header).set("label", args.title);
				...
				if (structure.layout.leftPane.hidden) {
					...
					registry.byNode(leftHeader).set("label", args.title);
				}
...
//				li.transitionTo(args.id);
	...
	return {
		...
		//locate to corresponding category
		var viewName = hash();
		if(viewName){
			var store = new dojo.store.Memory({
				data: {
					identifier: "id",
					items: structure.demos[0].views
				}
			});
			var viewConfig = store.get(viewName);
			if(viewConfig){
				setTimeout(function(){
					showView(viewConfig, registry.byId("controls"));
				}, 1);
			}
		}
	}
